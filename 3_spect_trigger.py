...
from PyQt5.QtWidgets import QApplication, QWidget, QPushButton, QVBoxLayout, QLabel, QSizePolicy, QHBoxLayout, QTextEdit, QMessageBox
...
class CameraApp(QWidget):
    ...
    def run_spectrometer(self):
        ...
        self.log("Starting spectrometer measurement (software trigger)...")
        self.snap_button.setEnabled(False)
        self.init_camera_button.setEnabled(False)
        self.init_spec_button.setEnabled(False)

        # Run measurement in a separate thread to keep GUI responsive,
        # but no QThread or signals — just threading.Thread + a callback with QTimer.singleShot
        def measure():
            try:
                AVS_PrepareMeasure(self.avs_handle, self.measconfig)

                # For software trigger, use 0 for trigger source in AVS_Measure
                AVS_Measure(self.avs_handle, 0, self.spec_num_scans)

                for i in range(self.spec_num_scans):
                    while not AVS_PollScan(self.avs_handle):
                        time.sleep(0.001)

                ret_arr, timestamp_arr, spectra_data_arr, wavelengths = avantes_readout(
                    self.measconfig.m_StopPixel + 1,
                    AVS_GetLambda(self.avs_handle),
                    self.spec_num_scans,
                    self.avs_handle,
                    self.spec_int_time
                )

                # Schedule GUI update in main thread via QTimer.singleShot
                def update_gui():
                    if spectra_data_arr:
                    ...
                    # Re-enable buttons
                    self.snap_button.setEnabled(True)
                    self.init_camera_button.setEnabled(True)
                    self.init_spec_button.setEnabled(True)
