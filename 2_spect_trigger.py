...
class CameraApp(QWidget):
    ...
    def take_snapshot(self):
        ...
        # Step 1: Inform user trigger is starting
        self.image_label.setText("Sending 5V trigger...")

        # Step 2: Send LabJack trigger
        send_trigger(pulse_us=100)  # 100μs active low
        print("Trigger sent, waiting 100μs before camera snapshot...")

        # Step 3: Wait 100μs
        time.sleep(0.0001)

        # Step 4: Camera snapshot logic
        def handler(cam: Camera, stream: Stream, frame: Frame):
            ...
            # Update UI
            self.image_label.setText("Snapshot taken. Starting spectrometer...")

            # Stop streaming safely after callback
            QTimer.singleShot(0, lambda: self.cam.stop_streaming())

            # Step 5: Wait ~200ms before spectrometer
            QTimer.singleShot(200, self.run_spectrometer)

        # Set camera trigger config before starting stream
    ...
    def run_spectrometer(self):
        try:
            ...
            # Begin measurement
            avantes_measure(handle, measconfig, self.spec_num_scans)

            # Readout result
            ret_arr, timestamp_arr, spectra_data_arr, wavelengths = avantes_readout(
                pixels, wavelength_calibration, self.spec_num_scans, handle, self.spec_int_time
            )

            self.image_label.setText("Snapshot and spectrum captured!")

            # You can plot or save spectra_data_arr as needed here
            print(f"Spectrum length: {len(spectra_data_arr[0])} data points")

        except Exception as e:
            self.image_label.setText(f"Error reading spectrometer:\n{e}")
